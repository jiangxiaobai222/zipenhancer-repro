...
class ActivationDropoutAndLinear(torch.nn.Module):
    ...
    def forward(self, x: Tensor):
        # FIX (training stability): the plain SwooshL/RForward use (1+x.exp()).log(),
        # whose BACKWARD overflows (x.exp()->inf->nan grad) — fine for inference (forward
        # is guarded) but diverges in training. Use the numerically-stable autograd
        # Functions (logaddexp forward + bounded quantized deriv backward), matching
        # icefall's k2-based stable swoosh. See docs/EXPERIMENTS.md fix #4.
        if self.activation == 'SwooshL':
            x = SwooshLFunction.apply(x)
        elif self.activation == 'SwooshR':
            x = SwooshRFunction.apply(x)
        else:
